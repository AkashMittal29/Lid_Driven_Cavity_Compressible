MODULE mod_read_input_file
    ...
    INTEGER, PARAMETER :: rkind = SELECTED_REAL_KIND(P=14,R=30)
    ...
    TYPE :: input_data_type
        ...
        CONTAINS
            ...
            PROCEDURE, PASS(self), PUBLIC :: assign_variable_real_array
            PROCEDURE, PASS(self), PUBLIC :: assign_variable_int_array
            GENERIC :: assign_variable => assign_variable_real, assign_variable_int, assign_variable_char, &
                                        & assign_variable_int_array, assign_variable_real_array ! For one dimensional array
    END TYPE
    ...
    CONTAINS
            ...
            DO WHILE(.TRUE.)
                ...
                ELSE IF(INDEX(text,'=')>0) THEN ! variable
                    ...
                    IF(INDEX(text2, ';')>0 .OR. INDEX(text2,'#')>0 .OR. INDEX(text2,'!')>0) THEN
                        ind = LEN_TRIM(text2) ! Initializing ind
                        ind1 = INDEX(text2, ';')
                        IF(ind1/=0) ind=ind1
                        ind1 = INDEX(text2, '#')
        ...
        SUBROUTINE assign_variable_real_array(self, sect_name, var_name, var)
            CLASS(input_data_type), INTENT(INOUT) :: self
            CHARACTER(LEN=*), INTENT(IN) :: sect_name, var_name
            REAL(KIND=rkind), INTENT(OUT), DIMENSION(:) :: var
            INTEGER :: index(2), sec_i, var_i

            index = self%get_index(sect_name, var_name); 
            sec_i = index(1); var_i = index(2);
            READ(self%sections(sec_i)%vars(var_i)%value,*) var
        END SUBROUTINE assign_variable_real_array
        ...
        SUBROUTINE assign_variable_int_array(self, sect_name, var_name, var)
            CLASS(input_data_type), INTENT(INOUT) :: self
            CHARACTER(LEN=*), INTENT(IN) :: sect_name, var_name
            ...
            sec_i = index(1); var_i = index(2);
            READ(self%sections(sec_i)%vars(var_i)%value,*) var
        END SUBROUTINE assign_variable_int_array
...
END MODULE mod_read_input_file
